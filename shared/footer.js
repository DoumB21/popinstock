'use strict';
/* Canonical site footer — one definition instead of a hand-copied <footer>
   block per page/nav.js. Every page that wants the footer calls this after
   loading the script, so a copy/legal/socials change is a one-file edit
   instead of touching every page.

   window.buildSharedFooter() — creates the <footer> element and appends it
   to document.body. Takes no arguments: every link here is an absolute URL,
   so there's no per-page path prefix to worry about (unlike wallet-menu.js's
   `prefix` param). */
window.buildSharedFooter = function () {
  const footer = document.createElement('footer');
  footer.innerHTML = `
    <div class="footer-socials">
      <a href="https://x.com/HoardioHQ" target="_blank" rel="noopener noreferrer" aria-label="X (Twitter)">
        <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.737-8.835L1.254 2.25H8.08l4.261 5.632 5.903-5.632zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
      </a>
      <a href="https://discord.gg/vzw45F5wnN" target="_blank" rel="noopener noreferrer" aria-label="Discord">
